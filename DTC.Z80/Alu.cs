...
public sealed class Alu
{
    ...
    internal byte SubtractAndSetFlags(byte a, byte b, bool subCf)
    {
        ...
        var needCarry = subCf && TheRegisters.Cf;
        if (needCarry)
            v--;

        var result = (byte) (v & 0xff);

        TheRegisters.Hf = IsHalfCarry8(a, b, needCarry, false);
        TheRegisters.Cf = IsCarry8(v);
        TheRegisters.Sf = !IsBytePositive(result);
        TheRegisters.Zf = v == 0;
        TheRegisters.Pf = IsOverflow8(a, b, result, false);
        TheRegisters.Nf = true;
    ...
    internal ushort SubtractAndSetFlags(int a, int b, bool subCf)
    {
        ...
        TheRegisters.Hf = IsHalfCarry16((ushort) a, (ushort) b, needCarry, false);
        TheRegisters.Cf = IsCarry16(v);
        TheRegisters.Sf = !IsWordPositive(result);
        TheRegisters.Zf = v == 0;
        TheRegisters.Pf = IsOverflow16((ushort)a, (ushort)b, result, false);
        TheRegisters.Nf = true;
    ...
    internal byte AddAndSetFlags(byte a, byte b, bool addCf)
    {
        ...
        var needCarry = addCf && TheRegisters.Cf;
        if (needCarry)
            v++;
        ...
        TheRegisters.Sf = !IsBytePositive(result);
        TheRegisters.Zf = result == 0;
        TheRegisters.Pf = IsOverflow8(a, b, result, true);
        TheRegisters.Hf = IsHalfCarry8(a, b, needCarry, true);
        TheRegisters.Cf = IsCarry8(v);
        TheRegisters.Nf = false;
        TheRegisters.SetFlags53From(result);
    ...
    internal ushort AddAndSetFlags(ushort a, ushort b, bool addCf)
    {
        ...
        if (addCf)
        {
            TheRegisters.Sf = !IsWordPositive((ushort)v);
            TheRegisters.Zf = result == 0;
            TheRegisters.Pf = IsOverflow16(a, b, result, true);
        }

        TheRegisters.Hf = IsHalfCarry16(a, b, needCarry, true);
        TheRegisters.Cf = IsCarry16(v);
        TheRegisters.Nf = false;
        TheRegisters.SetFlags53From(result);
    ...
    public byte DecAndSetFlags(byte b)
    {
        ...
        TheRegisters.Hf = IsHalfCarry8(b, 0x01, false, false);
        TheRegisters.Sf = !IsBytePositive(result1);
        TheRegisters.Zf = (byte)v == 0;
        TheRegisters.Nf = true;
        var result = (byte)v;
        TheRegisters.Pf = b == 0x80;
        TheRegisters.SetFlags53From(result);
        return result;
    }
    ...
    public byte IncAndSetFlags(byte b)
    {
        int v = b;
        v += 0x01;
        TheRegisters.Hf = IsHalfCarry8(b, 0x01, false, true);
        TheRegisters.Sf = !IsBytePositive((byte)v);
        TheRegisters.Zf = (byte)v == 0;
        TheRegisters.Nf = false;
        TheRegisters.Pf = b == 0x7F;
        TheRegisters.SetFlags53From((byte)v);
    ...
    public void And(byte b)
    {
        ...
        TheRegisters.Sf = !IsBytePositive(r);
        TheRegisters.Zf = r == 0;
        TheRegisters.Hf = true;
        TheRegisters.Pf = IsEvenParity(r);
        TheRegisters.Nf = false;
        TheRegisters.Cf = false;
    ...
    public void AdjustAccumulatorToBcd()
    {
        ...
        if (TheRegisters.Cf) lookupIndex |= 256;
        if (TheRegisters.Hf) lookupIndex |= 512;
        if (TheRegisters.Nf) lookupIndex |= 1024;
    ...
    internal byte RotateLeft(byte b)
    {
        ...
        var oldCarry = (byte)(TheRegisters.Cf ? 1 : 0);
        b <<= 1;
        b |= oldCarry;
        TheRegisters.Sf = !IsBytePositive(b);
        TheRegisters.Zf = b == 0;
        TheRegisters.Hf = false;
        TheRegisters.Pf = IsEvenParity(b);
        TheRegisters.Nf = false;
        TheRegisters.Cf = oldHighBit != 0x00;
